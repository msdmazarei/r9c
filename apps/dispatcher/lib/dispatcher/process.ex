...
defmodule Dispatcher.Process do
  ...
  @process_nodes ["n2@s1.kafka.local", "n3@s1.kafka.local"]
  ...
  def send_bunch_of_messages_locally(process_message_tuple_list) do
    ...
    events =
      ...
      |> Enum.map(fn {pn, msg} ->
        ...
        event_log(str_e_type, str_id, "send_message", process_name)
      end)
    ...
    failed_deliv_events =
      ...
      |> Enum.map(fn {pn, msg} ->
        ...
        process_name = pn |> Utilities.to_string()
        event_log(str_e_type, str_id, "delivery_failed", process_name)
      end)

    EventLogger.log_events(failed_deliv_events)
    r
  end
