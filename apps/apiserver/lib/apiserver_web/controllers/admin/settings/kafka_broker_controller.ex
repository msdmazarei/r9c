defmodule ApiserverWeb.Admin.Settings.Kafka.Broker.Controller do
  ...
  require DatabaseEngine.Interface.SystemConfig.KafkaBroker.Repo
  alias DatabaseEngine.Interface.SystemConfig.KafkaBroker.Repo
  ...
  def edit(conn, params) do
    ...
    {status, rtn} =
      case Repo.edit(model_instance) do
        {:aborted, :conflict} ->
          Logging.info("version conflict. broker:~p", [model_instance.host])
          {409, %{"version" => "conflict"}}

        {:aborted, :not_exist} ->
          Logging.info("node not exists. broker:~p", [model_instance.host])
          {404, nil}

        {:aborted, :invalid_model} ->
          Logging.info("bad request body to edit broker:~p", [model_instance.host])
        ...
        {:atomic, model} ->
          Logging.info("successfully edited. broker:~p", [model_instance.host])
          {200, model |> Map.from_struct()}
      end
